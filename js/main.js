...
var elem;
if(jQuery('.selectothers').length > 0){
   $( ".selectothers" ).selectmenu({
  open: function( event, ui ) {
      //alert(JSON.stringify(ui.html()));
      /*elem=$(this);
      name=elem.attr('name');
      innerElement=$('ul#'+name+'-menu li:last-child');
      if(innerElement.find('input').length == 0){
          event.preventDefault()
          innerElement.find('div').addClass('others');
          innerElement.find('div').text('').append('<input class="form-control other-text" placeholder="other '+name+'" type="text" />');
      }*/
...
$('#selectmodal').on('hidden.bs.modal', function (e) {
  ...
  text_value=$('#selectmodal').find('.form-control').val();
  elem.find('option:last-child').attr('selected','selected').val(text_value);
  $('#'+elem.attr('name')+'-button .ui-selectmenu-text').text(text_value)
})
    
}

$( ".selectothers" ).on( "selectmenuselect", function( event, ui ) {
    if(ui.item.value=="others"){
      elem=$(this);
      name=elem.attr('name');
      $('#selectmodal').find('.title-label span').text(name);
      $('#selectmodal').find('.form-control').val('');
...
jQuery('document').ready(function(){
    ...
    if($('#post-upload').length > 0){
         $("#redBecomesBlue").percircle({percent:1,text: "0",progressBarColor:"#6b3096"});
        
        $('#post-upload .upload-btn').click(function(events){
            events.preventDefault();
            if($('.upload-cont .thumb-list').length > 0){
                 $('.status-cont p:last-child').removeClass('error').addClass('success').append('<span>100%</span>');
                changeCircle(100);
                $('#post-upload').submit();
            }else{
                $('.status-cont p:last-child').addClass('error').append(' <span>0%</span>');
                $('<label class="error upload-error">Please you need to upload an image to finish this step</label>').insertAfter(' .upload-btn-cont');
            }
        })
    }
    
    if($("#post-p").length > 0){
        jQuery(".auto-com input").click(function(){
         if (jQuery(".auto-com input").is(":checked")) {
               $('#descrip-text').val(jQuery(".auto-com input").val());
          }else{
              $('#descrip-text').val('')
          }
    })
        $( "select#pay_type" ).on( "selectmenuselect", function( event, ui ) {
            item_val=ui.item.value;
            if(item_val=="installments"){
                $("#pay_dur, #init_dep").selectmenu("enable");
            }else{
                 $("#pay_dur, #init_dep").selectmenu("disable");
            }
        } );
